...
cookie, access_token = siem_connection.login("oanhptk6", "123qweA@")
# print(siem_rule.search_rule("T1105_Ingress_Tool_Transfer_ver1", cookie).json()["data"][0]["engines_filter"]["content"])
...
def init_md(platform, filename):
    ...
    for i in range(2, ws.max_row + 1):
        ...
        #Rule(rule_name, tag)
        #rulename = ...
        #rule_json = siem_rule.search_rule(rulename, cookie).json()["data"][0]
        
        rule_name = str(ws.cell(row=i,column=1).value).strip()
        rule_tag = str(ws.cell(row=i,column=3).value).replace(' ', '').split(",")
        status = siem_rule.search_rule(rule_name, cookie)
        rule_json = status
        if(len(rule_json)==0):
            print("Not found " + rule_name + " " + str(i-1))
            continue
